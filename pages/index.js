...
export default function LeaseGuard() {
  ...
  useEffect(() => {
    ...
    if (rId && payment === "success" && success === "true") {
      // After Stripe redirect, report is in Redis — fetch it from the server
      setScreen("loading");
      setLoadingStep(4);
      fetch(`/api/report?reportId=${rId}`)
        .then(res => res.json())
        .then(data => {
          if (data.report) {
            ...
            setError("Could not load your report. Please contact support.");
            setScreen("home");
          }
        })
        .catch(() => {
